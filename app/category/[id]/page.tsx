...
function getOsFromBrand(brand: string): string {
  if (brand === "Apple") return "iOS";
  if (brand === "Samsung" || brand === "Google") return "Android";
  return "Android";
}
...
export default async function CategoryPage({
  ...
  const chipsetFilter = typeof sp.chipset === "string" ? sp.chipset : undefined;
  const osFilter = typeof sp.os === "string" ? sp.os : undefined;
  ...
  const filteredItems = allProducts.filter((p) => {
    ...
    if (chipsetFilter && p.specs.Chipset !== chipsetFilter) return false;
    if (osFilter) {
      if (osFilter === "iPhone" && p.brand !== "Apple") return false;
      if (osFilter === "Android" && p.brand === "Apple") return false;
    }
    return true;
  });
